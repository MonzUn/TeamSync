...
const MEngine::TextBoxEditFlags TEXT_BOX_EDIT_OVERWRITE_RESET_FLAG = (MEngine::TextBoxEditFlags::Editable | MEngine::TextBoxEditFlags::OverwriteOnDefaultTextMatch | MEngine::TextBoxEditFlags::ResetToDefaultWhenEmpty); // TODODB: Why can't this be a constexpr (Causes error: "Expression must have constant value")? Everything seems to be const...
...
namespace MEngine
{
	...
	EntityID CreateTextBox(int32_t posX, int32_t posY, int32_t width, int32_t height, MEngineFontID fontID, uint32_t posZ = MENGINE_DEFAULT_UI_TEXTBOX_DEPTH,
		const std::string& text = "", TextAlignment alignment = TextAlignment::CenterLeft, TextBoxEditFlags editFlags = TextBoxEditFlags::None, const ColorData& backgroundColor = PredefinedColors::Colors[PredefinedColors::TRANSPARENT],
